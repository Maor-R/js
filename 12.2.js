...
function getCandy(candyStore, id) {
  ...
  candyStore.candies.forEach(element => {
    if(element.id == id) retCandy = element; 
  });
  return retCandy;
}
...
function getPrice(candyStore, id) {
    let retCandyPrice ;
  candyStore.candies.forEach(element => {
    if(element.id == id) retCandyPrice = element.price; 
  });
  return retCandyPrice;
}
...
addCandy(candyStore, "abcde", "test", 6);
...
function buy(candyStore, id) {
    ...
    const amount = candy.amount;
    candyStore.cashRegister +=amount * getPrice(candyStore, id);
    candy.amount = 0;
